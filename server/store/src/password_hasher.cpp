...
// A textbook SHA-256 (FIPS 180-4). Confined to this file: it exists only to hash
// a password, so it is a private implementation detail of hashPassword, not a
// general crypto module the rest of the code may reach for.
...
std::string sha256Hex(const std::string& message) {
    ...
    for (int shift = 56; shift >= 0; shift -= 8) {
        data.push_back(static_cast<std::uint8_t>((bitLength >> shift) & 0xff));
    }
    ...
    for (std::size_t chunk = 0; chunk < data.size(); chunk += 64) {
        ...
        for (std::size_t i = 0; i < 64; ++i) {
            ...
            const std::uint32_t t1 = hh + S1 + ch + roundConstants[i] + w[i];
...
// The separator folded between username and password, so "ab"+"c" and "a"+"bc"
// cannot collide onto the same digest.
constexpr char saltSeparator = ':';

}  // namespace

std::string hashPassword(const std::string& username,
                         const std::string& password) {
    return sha256Hex(username + saltSeparator + password);
}
